...
export function init() {
  ...
  window.p2p = {
    ...
    setupConn(conn) {
      ...
      // 超限熔断：如果是主动连接且满员，拒绝 (被动连接通常放行，或者在maintenance清理)
      if (Object.keys(window.state.conns).length >= max + 50) { // 留50个缓冲区
        conn.close();
        return;
      }
